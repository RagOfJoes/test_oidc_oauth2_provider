...
const { DB_URI, DB_NAME } = require('../lib/constants');
...
(async () => {
	if (!client) {
		try {
			client = await mongodb.connect(DB_URI, { useUnifiedTopology: true });
...
class MongoAdapter {
	...
	async upsert(_id, payload, expiresIn) {
		...
		await this.coll().updateOne({ _id }, { $set: { payload, ...(expiresAt ? { expiresAt } : undefined) } }, { upsert: true });
	}

	async find(_id) {
		const result = await this.coll()
			.find({ _id }, this.findOneOptions)
			.limit(1)
			.next();
