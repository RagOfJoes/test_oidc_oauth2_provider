...
module.exports = (server, provider) => {
	...
	server.post('/interaction/:uid/signup', setNoCache, body, async (req, res, next) => {
		try {
			const { uid, prompt, params, session } = await provider.interactionDetails(req, res);
			const client = await provider.Client.find(params.client_id);
	...
	server.post('/interaction/:uid/login', setNoCache, body, async (req, res, next) => {
		try {
			const { uid, prompt, params, session } = await provider.interactionDetails(req, res);
			const client = await provider.Client.find(params.client_id);
			...
			try {
				if (req.body.new_user) {
					result = { select_account: {}, sign_up: {}, meta: { newUser: true } };
				} else {
	...
	server.post('/interaction/:uid/continue', setNoCache, body, async (req, res, next) => {
		try {
	...
	server.post('/interaction/:uid/confirm', setNoCache, body, async (req, res, next) => {
		try {
			const {
				prompt: { name }
	...
	server.get('/interaction/:uid/abort', setNoCache, async (req, res, next) => {
		try {
			const result = {
				error: 'access_denied',
